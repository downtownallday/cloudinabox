...
tag_from_readme() {
    TAG=$(grep -F "git checkout" "$miabdir/README.md" | head -1 | sed 's/^.*git checkout \([0-9a-zA-Z\.]*\).*$/\1/')
    if [ $? -ne 0 -o -z "$TAG" ]; then
        echo "Could not determine code version from README.md !!!" 1>&2
        return 1
    fi    
}
...
create_changelog() {
    ...
    echo "| ------ | ---- | ------ | ----- |"
    git log --no-merges --format="| [%h](https://github.com/downtownallday/$project/commit/%H) | %cs | _%an_ | %s |" $from_ref..$to_ref
}
...
EOF
    ...
    tag_from_bootstrap || tag_from_readme || exit 1
    bs_tag="$TAG"
    echo -n "found tag $bs_tag: "
